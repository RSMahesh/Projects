...
namespace WebServer
{
    public class WebServer : IWebServer
    {
            ...
            while (_listener.IsListening)
            {
                ThreadPool.QueueUserWorkItem((c) =>
                {
                    ...
                    try
                    {
                        ...
                        ctx.Response.Headers["Access-Control-Allow-Headers"] = "authorization,content-type,filename";
         ...
         IHttpHandler GetHttpHandler(HttpListenerRequest request)
        {
            ...
            {
                return new HttpHandlers.PostRequestHttpHandler (_webSitePhysicalPath);

            }

            if (request.HttpMethod == "OPTIONS")
            {
                return new HttpHandlers.OptionsRequestHttpHandler(_webSitePhysicalPath);
